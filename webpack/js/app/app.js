...
$(document).on('click', '#daynight', function () {
  // var SessionStore = new Store('erp-db', 'session')
  get('theme', SessionStore).then( result => {
    console.log("myre: ", result);
    if (result != undefined) {
//Get current theme
if (result == 'dark') {
  //set light
  set('theme', 'light', SessionStore)
      .then((val) => {console.log(val); location.reload();});

}
if (result == 'light')  {
  //set dark
  set('theme', 'dark', SessionStore)
      .then((val) =>   {console.log(val);darktheme()});
